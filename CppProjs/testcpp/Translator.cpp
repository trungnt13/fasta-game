...
string Translator::translate(string number) {
  ...
  vector<string> mainNumber = this->validateNumber(number);
  ...
  if (mainNumber[0] == "-") num[0] = this->dict["-"];

  /** parse number string **/
  for (int i = 1; i < min(sizeof(num), mainNumber.size()); i++) {
    string currentNumber = mainNumber[i];
    ...
    if (i == 2 && currentNumber[0] == '0' && slicedNumber.size() > 0) {
      while (mainNumber[i][0] == '0') {
        appendString += this->dict["0"] + " ";
        // remove first character
        mainNumber[i] = mainNumber[i].substr(1, mainNumber[i].length());
      }
      slicedNumber = sliceReversedString(mainNumber[i], 3);
    }
    ...
    if (slicedNumber.size() > 0) {
      ...
      for (int j = 1; j < translatedSlicedNumber.size(); j++) {
        if (translatedSlicedNumber[j].length() > 0) {
          string tmp("1");
          for (int k = 0; k < j; k++) tmp += "000";
          num[i].insert(
              0, translatedSlicedNumber[j] + " " + this->dict[tmp] + " ");
        }
      }
    }

    // = if above exception happen =
  ...
  try {
    ...
    // = read 10-99 =
      ...
      if (n < 20)
        ...
        int b = n - a;
        if (b != 0)
          result += this->dict[toString(a)] + "-" + this->dict[toString(b)];
        else
          result += this->dict[toString(a)];
      }
    }
    // = read 100-999 =
    else {
      ...
      int b = n - a * 100;
      if (b != 0)
        result += this->dict[toString(a)] + " " + this->dict[toString(100)] +
