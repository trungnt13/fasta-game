...
public class Button extends Widget implements Disableable {
	private TButtonStyle style;
	...
	public Button (Skin skin) {
		initialize();
		setStyle(skin.get(TButtonStyle.class));
	...
	public Button (Skin skin, String styleName) {
		initialize();
		setStyle(skin.get(styleName, TButtonStyle.class));
		setWidth(getPrefWidth());
		setHeight(getPrefHeight());
	}

	public Button (TButtonStyle style) {
		initialize();
		setStyle(style);
		setWidth(getPrefWidth());
		setHeight(getPrefHeight());
	...
	public Button (Drawable up) {
		this(new TButtonStyle(up, null, null));
	}

	public Button (Drawable up, Drawable down) {
		this(new TButtonStyle(up, down, null));
	}

	public Button (Drawable up, Drawable down, Drawable checked) {
		this(new TButtonStyle(up, down, checked));
	}
	...
	public void setStyle (TButtonStyle style) {
	...
	/** Returns the button's style. Modifying the returned style may not have an effect until {@link #setStyle(TButtonStyle)} is
	 * called. */
	public TButtonStyle getStyle () {
		return style;
	}
	...
	public void draw (SpriteBatch batch, float parentAlpha) {
		...
		if (background != null) {
			...
			if (background.getClass() == TextureRegionDrawable.class) {
					TextureRegion region = ((TextureRegionDrawable)background).getRegion();
					if (scaleX == 1 && scaleY == 1 && rotation == 0)
						batch.draw(region, x, y);
					else {
						batch.draw(region, x, y, getOriginX(), getOriginY(), region.getRegionWidth(), region.getRegionHeight(),
							scaleX, scaleY, rotation);
					}
				} 
//				 	else	background.draw(batch, x, y, imageWidth * scaleX, imageHeight * scaleY);
//			background.draw(batch, getX(), getY(), getWidth(), getHeight());
	 ...
	 * @author mzechner */
	static public class TButtonStyle {
		/** Optional. */
		...
		public TButtonStyle (Drawable up, Drawable down, Drawable checked) {
			this.up = up;
			this.down = down;
			this.checked = checked;
		}

		public TButtonStyle (TButtonStyle style) {
			this.up = style.up;
