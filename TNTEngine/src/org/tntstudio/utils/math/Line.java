...
public class Line implements Disposable {
	...
	public Vector2 getMidPoint () {
		return mTMP.set((lineStart.x + lineEnd.x) / 2, (lineStart.y + lineEnd.y) / 2);
	}
	...
	public float module () {
		float tmp = (float)Math.sqrt((lineStart.x - lineEnd.x) * (lineStart.x - lineEnd.x) + (lineStart.y - lineEnd.y)
			* (lineStart.y - lineEnd.y));
		return tmp;
	}

	public float module2 () {
		float tmp = (float)((lineStart.x - lineEnd.x) * (lineStart.x - lineEnd.x) + (lineStart.y - lineEnd.y)
			* (lineStart.y - lineEnd.y));
		return tmp;
	}
	...
	public boolean isOnLeftSide (Vector2 point) {
		if (getSideOnLine(point) < 0) return true;
		return false;
	}
	...
	public Vector2 calCrossPoint (Line line) {
		...
		if (a1 * b2 == b1 * a2)
			...
			return new Vector2((b1 * c2 - c1 * b2) / (b2 * a1 - b1 * a2), (a2 * c1 - c2 * a1) / (b2 * a1 - b1 * a2));
	}
