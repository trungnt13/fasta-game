...
public final class Debug {
	...
	/********************************************************************
         * 
         ********************************************************************/
	public static void array (short[] i) {
		if (!isEnable) return;
	...
	public static void array (Object... i) {
		if (!isEnable) return;
	...
	public static void array (boolean[] b) {
		if (!isEnable) return;
	...
	public static void array (String[] b) {
		if (!isEnable) return;
	...
	public static void array (long[] l) {
		if (!isEnable) return;
	...
	public static void array (byte[] l) {
		if (!isEnable) return;
	...
	public static void list (Iterable it) {
	...
	public static void out (Vector2... v) {
		...
		if (v[0] == null) Debug.out("NULL");
		StringBuilder tmp = new StringBuilder();
		for (int i = 0; i < v.length; i++) {
			tmp.append("\n Vector2[" + i + "]  : " + v[i].x + "   " + v[i].y);
		}
		Debug.out(tmp.toString());
	}
	...
	public static void nativeHeap (int type) {
		...
		switch (type) {
		case MB:
			Gdx.app.log("Native Heap : ", "" + Gdx.app.getNativeHeap() / 1024 / 1024);
			break;
		case GB:
			Gdx.app.log("Native Heap : ", "" + Gdx.app.getNativeHeap() / 1024 / 1024 / 1024);
			break;
		case KB:
	...
	public static int fps () {
		...
		if (log == null) log = new eFPSLogger();
		return log.log();
	}
	...
	public final static void printConstructors (Class type) {
		...
		for (Constructor ctor : allConstructors) {
			...
			for (int i = 0; i < pType.length; i++) {
				Debug.out(ctor.toGenericString());

				Type[] gpType = ctor.getGenericParameterTypes();
				for (int j = 0; j < gpType.length; j++) {
					Debug.out("GenericParameterType" + j + " " + gpType[j]);
				}
				break;
			}
		}
	...
	public final static void bugHere () {
		Debug.out(" has a Bug here!");
	}

	public final static void shitoverhere () {
		Debug.out("   Shit over here   ");
	}

	public final static void damnoverhere () {
		Debug.out("   Damn over here   ");
	}
	...
	public static void currentThread () {
		Debug.out(Thread.currentThread().getId() + "  " + Thread.currentThread().getName() + "  "
