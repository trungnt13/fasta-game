...
import org.tntstudio.utils.math.Line;
...
public final class Utils {
	public static final boolean isSuperAndChild (Class superClass, Class childClass) {
		return superClass.isAssignableFrom(childClass);
	}

	public static final boolean isSuperAndChild (Object supers, Object child) {
		return supers.getClass().isAssignableFrom(child.getClass());
	}
...
// Gdx.app.log("shit", "" + a + " " + b + " " + c + "  " + a1 + " " + b1 + " " + c1);
	...
	/** if (val > max) => val = max <li>
	 * if (val < min) => val = min <li>
	 * return val; */
	public static float boundTheValue (float max, float currentValue, float min) {
		if (max < min) {
			float tmp;
			tmp = max;
			max = min;
	...
	public static String getIdentification (Object o) {
